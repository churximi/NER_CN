...
from tensorflow.contrib.layers.python.layers import initializers  # 初始化
...
class Model:
    ...
    def __init__(self, config):
        ...
        self.char_dim = config["char_dim"]  # 词向量维度
        self.lstm_dim = config["lstm_dim"]  # lstm维度
        self.seg_dim = config["seg_dim"]  # 分词维度

        self.num_tags = config["num_tags"]  # 标签种类数
        self.num_chars = config["num_chars"]  # 词种类数
        self.num_segs = 4

        # 变量
        self.global_step = tf.Variable(0, trainable=False)
        self.best_dev_f1 = tf.Variable(0.0, trainable=False)  # 最好的dev_f1值
        self.best_test_f1 = tf.Variable(0.0, trainable=False)  # 最好的test_f1值

        self.initializer = initializers.xavier_initializer()  # Xavier初始化

        # 添加placeholders
        self.char_inputs = tf.placeholder(dtype=tf.int32, shape=[None, None],
                                          name="ChatInputs")
        self.seg_inputs = tf.placeholder(dtype=tf.int32, shape=[None, None],
                                         name="SegInputs")
        ...
        used = tf.sign(tf.abs(self.char_inputs))  # 先取绝对值使>=0，再转为0,1【tf.sign转为-1,0,1】
        ...
        embedding = self._embedding_layer(self.char_inputs, self.seg_inputs, config)
    ...
    def _embedding_layer(self, char_inputs, seg_inputs, config):
        """
        ...
        with tf.variable_scope("char_embedding"), tf.device('/cpu:0'):
            self.char_lookup = tf.get_variable(
                name="char_embedding",
                shape=[self.num_chars, self.char_dim],  # 词种类数，词向量维度
                initializer=self.initializer)  # 初始化
    ...
    def biLSTM_layer(self, lstm_inputs, lstm_dim, lengths):
        ...
        [batch_size, num_steps, 2*lstm_dim]
        """
